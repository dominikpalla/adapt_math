...
KNOWLEDGE_WEIGHTS — plochý seznam **76** všech vah znalostního vektoru
...
V UI rozlišujeme **dvě věci**:
  ...
  1) Vektor znalostí (`KNOWLEDGE_WEIGHTS`, 76) — celý mix vlastností,
...
assert len(KNOWLEDGE_WEIGHTS) == 76, f"Očekáváno 76 vah, je {len(KNOWLEDGE_WEIGHTS)}"
...
assert len(TASK_CATEGORIES) + len(TASK_PROPERTIES) + len(TASK_TYPES) + len(TASK_SKILLS) \
       == len(KNOWLEDGE_WEIGHTS), "Suma anotačních podmnožin != 76"

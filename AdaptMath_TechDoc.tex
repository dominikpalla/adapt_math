...
\subsection{KNOWLEDGE\_WEIGHTS (61 vah, verze 260601\_v2)}

Definováno v~\texttt{tasks/knowledge\_weights.py}. Seznam byl revidován dr.~Medkovou (verze \texttt{260601\_v2}) a obsahuje pouze \emph{listové} váhy --- hlavní (header) kategorie z původního excelu nejsou v seznamu. Všechny váhy jsou seskupené do 13 \emph{skupin} pro účely barevného rozlišení v UI a budoucí paušální distribuce.

\paragraph{Skupiny vah (13).} Každá váha nese prefix tvaru \texttt{Skupina - Název} (např. \texttt{"Limita - LVL (krácení)"}). Položky bez prefixu (\emph{Konvexnost/konkávnost}, \emph{Průběh funkce}) mají vlastní samostatnou skupinu.
...
\texttt{KNOWLEDGE\_WEIGHTS} (61) je v \texttt{tasks/knowledge\_weights.py} rozdělen do čtyř disjunktních anotačních podmnožin:

\begin{itemize}
  \item \texttt{TASK\_CATEGORIES} (45) --- SŠ + Funkce + Monotonie + Konvexnost + Spojitost + Limita + Derivace + Průběh funkce + Primitivní funkce + Určitý integrál. Použito jako \emph{single-select} v UI: jedna primární kategorie úlohy.
  \item \texttt{TASK\_PROPERTIES} (9) --- skupina \emph{Vlastnosti}. \emph{Multi-select} (checkboxy).
  \item \texttt{TASK\_TYPES} (1) --- skupina \emph{Typ} (zatím jen \emph{Aplikační}). \emph{Multi-select}.
  \item \texttt{TASK\_SKILLS} (6) --- skupina \emph{Dovednosti}. \emph{Multi-select}.
\end{itemize}

Součet $45 + 9 + 1 + 6 = 61$ pokrývá celý \texttt{KNOWLEDGE\_WEIGHTS} (sanity-check při importu modulu).
...
Plný seznam 61 vah ve verzi \texttt{260601\_v2}, seskupený dle prefixu. Pořadí v tabulce odpovídá pořadí ve zdrojovém kódu i v UI.
...
\begin{tabular}{lll}
  ...
  \texttt{GET}        & \texttt{/tasks}           & Tabulka všech úloh (filtr podle kategorie, fulltext). \\
...
\paragraph{Filtr.} Dropdown nad tabulkou nabízí všech 61 kategorií (text option barevně podle skupiny) plus speciální položku „--- bez kategorie ---" pro rychlou identifikaci úloh, které tým ještě nezpracoval. Fulltext input hledá v \texttt{task\_id}, hodnotě kategorie i v \texttt{content\_latex}.
...
\begin{itemize}
  ...
  \item \emph{Kategorie úlohy} --- single-select 45 položek (jen SŠ + VŠ matematika), text barevně rozlišený podle příslušné skupiny.
  \item Pod selektem 3 \emph{multi-select} sekce s checkboxy --- jsou barevně sladěné se skupinou:
  \begin{itemize}
    \item \emph{Vlastnosti} (9 položek, zelená),
    \item \emph{Typ} (zatím 1 položka --- Aplikační, krémová),
    \item \emph{Dovednosti} (6 položek, oranžová).
  \end{itemize}
  Hodnoty se ukládají do JSON sloupců \texttt{properties}, \texttt{task\_type}, \texttt{skills}.
...
\begin{enumerate}
  ...
  \item \textbf{Distribuce vah dle kategorie.} Definovat slovník \texttt{CATEGORY\_DEFAULT\_VECTOR}, který pro každou ze 61 kategorií předvyplní výchozí distribuci 61 vah. Při výběru kategorie v UI se předvyplní \texttt{knowledge\_vector}, expert pak ladí.
...
\begin{enumerate}
  ...
  \item \textbf{Studentský profil.} Tabulka \texttt{students} s preferencemi (styl učení, motivace, math anxiety) a vektorem znalostí (61 hodnot v $[0, 1]$).
